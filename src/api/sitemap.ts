...
const {
  SITEMAP_PAGESIZE: pageSize = 100 as number,
...
function generateLoc(item: PrismicDocument): string | undefined {
...
function calculateChangeFrequency(item: PrismicDocument): string | undefined {
...
function calculatePriority(item: PrismicDocument): string | undefined {
  if (item.__typename === 'Frontpage') {
    return '1.0';
  }
...
function generateLastModified(item: PrismicDocument): string | undefined {
...
async function fetchPage(page: number, lang: PrismicLocale): Promise<Result | null> {
  ...
  try {
    ...
    const mappedResults: Array<SitemapUrl> = (pages?._allDocuments.edges || [])
      .map((result) => {
        const node = result?.node as PrismicDocument | undefined;

        if (!node || !node.__typename || skipTypesArray.includes(node.__typename)) {
          return null;
        }
